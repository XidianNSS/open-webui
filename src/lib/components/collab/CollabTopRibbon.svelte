<script lang="ts">
	...
	if (typeof window !== 'undefined') {
		skipEntranceAnimation = sessionStorage.getItem('skipCollabRibbonAnimation') === '1';
					...
					<div class="grid gap-3 md:grid-cols-2">
						...
						>
							...
							<div class="mt-3 h-2.5 overflow-hidden rounded-full bg-[#EAF0F6] dark:bg-white/8">
								<div
									class={`h-full rounded-full ${motionClass}`}
					...
					>
						...
						>
							...
							>
								...
								<div
									class={`absolute inset-y-0 left-0 bg-[linear-gradient(90deg,#39B5FF_0%,#B9E8D8_100%)] ${motionClass}`}
									style={`width:${edgeWidth}%;opacity:${hasStrategyPercent ? 1 : 0};`}
								></div>

								<div
									class={`absolute inset-y-0 right-0 bg-[linear-gradient(90deg,#FF9F31_0%,#F4C35E_100%)] ${motionClass}`}
									style={`width:${cloudWidth}%;opacity:${hasStrategyPercent ? 1 : 0};`}
								></div>

								<div
									class={`pointer-events-none absolute inset-0 opacity-15 ${opacityMotionClass}`}
								...
								<div
									class={`pointer-events-none absolute inset-y-1.5 z-[2] w-[2px] rounded-full bg-white/90 shadow-[0_0_0_1px_rgba(255,255,255,.45)] ${motionClass}`}
									style={`left: calc(${edgeWidth}% - 1px);opacity:${hasStrategyPercent && edgeWidth > 0 && cloudWidth > 0 ? 1 : 0};`}
								></div>

								<div class="relative flex h-14">
									<div
										class={`flex items-center px-5 text-[13px] font-semibold text-white ${motionClass}`}
									...
									<div
										class={`flex items-center justify-end px-5 text-[13px] font-semibold text-white ${motionClass}`}
